...
public class Entorno {
    ...
    public Entorno(int[][] mapa, int ox, int oy) {
        map = mapa;
    ...
    public void setDestino(int dx, int dy){
        ...
        mapa_heu = new Nodo[map.length][map[0].length];
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[i].length; j++) {
                mapa_heu[i][j] = new Nodo(i, j, map[i][j], false);
    ...
    public void mostrarEstado(ArrayList<Nodo> v) {
        ...
        marco.setTitle("Simulación");
        marco.setSize(800, 800);
        
        MapPreviewPanel mapav = new MapPreviewPanel(this);
        mapav.setMapMatrix(map);
        mapav.setsim();
        marco.add(mapav);
        mapav.repaint();
        //System.out.println("PASOS: " + cont + "\n");
